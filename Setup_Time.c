...
extern unsigned char second, minute,            // time variables
        hour, dow, day, month, year;   
extern unsigned char setup_second, setup_minute,
        setup_hour, setup_day, setup_month, setup_year;
...
extern char *nbr;
extern char *txt;
extern char tempC[];                            // temperature
extern char tempF[];                            // temperature                            
extern char time[];
extern char date[];
extern char found;
extern short Nec_OK ;
...
void Do_Setup_Time(void)
{
    ...
    {   
         if (check_for_button_input() == 1)     // certain commands linked to the controller button
         {

             if ((found == Prev) || (found == Next) || (found == Ch_Minus) || (found == Ch_Plus) || (found == Minus) || (found == Plus))
             {
                Do_Beep_Good();                 // command  recognized
                if (found == Prev) Go_Prev_Field();
                if (found == Next) Go_Next_Field();
                if (found == Ch_Plus) Do_Save_New_Time();
                if (found == Ch_Minus) Exit_Time_Setup(); 
             ...
             {
                 Do_Beep_Bad();                 //command not recognized
             }
        }                   
    }
}
...
void Increase_Time()                                  
{
            ...
            {
                case 0:                         // if the hour exceeds 24 it rolls back over to 0
                    setup_hour++;
                ...
                case 2:                         //second increment
                    setup_second++;
                    if (setup_second == 60) setup_second = 0; 
                    break;
                    
                case 3:                         //month increment
                    setup_month++;
                    if (setup_month == 13) setup_month = 1; 
                    break;
                    
                case 4:                         //day increment  
                    setup_day++;
                    if (setup_day > 31) setup_day = 1; 
                    break;   
                    
                case 5:                         //year increment
                    setup_year++;
                    if (setup_year>=99) setup_year = 0; 
                    break;                       
...
void Go_Next_Field()
{
    ...
    Update_Setup_Screen_Cursor_Forward(Select_Field);   
...
void Go_Prev_Field()
{
    ...
    Update_Setup_Screen_Cursor_Backward(Select_Field);  
} 
...
{ 
    ...
    strcpy(txt, "ECE3301L Sp21 Final\0");       // Text displayed 
...
{
    setup_time[0]  = (setup_hour/10)  + '0';     // MSB
    setup_time[1]  = (setup_hour%10)  + '0';     // LSB
    setup_time[3]  = (setup_minute/10) + '0';    // MSB
    setup_time[4]  = (setup_minute%10) + '0';    // LSB             
    setup_time[6]  = (setup_second/10) + '0';                    
    setup_time[7]  = (setup_second%10) + '0';                     
    setup_date[0]  = (setup_month /10) + '0';                   
    setup_date[1]  = (setup_month %10) + '0';                    
    setup_date[3]  = (setup_day/10) + '0';      // MSB                     
    setup_date[4]  = (setup_day%10) + '0';      //LSB                 
