...
void Do_Setup_Alarm_Time(void)
{
    ...
    {   
         if (check_for_button_input() == 1)             // setting the commands for specified buttons
         {
             if ((found == Prev) || (found == Next) || (found == Ch_Minus) || (found == Ch_Plus) || (found == Minus) || (found == Plus))
             {
                ...
                if (found == Ch_Plus) Do_Save_New_Alarm_Time();
                if (found == Ch_Minus) Exit_Setup_Alarm_Time();
...
void Increase_Alarm_Time()                                  // when incrementing from the max value  it loops back to the starting point
{
...
{       
            ...
            {
                case 0:                                     // when decrementing from starting point it loops back to the end point
...
void Go_Next_Alarm_Field()                                  // change menus when in alarm setting
{
...
void Go_Prev_Alarm_Field()                                  // change menus when in alarm setting
{
...
void Exit_Setup_Alarm_Time()                                // exit the alarm menu
{
    printf ("Exit Alarm");
    DS3231_Read_Alarm_Time();                               // Read alarm time
...
void Do_Save_New_Alarm_Time()                               // save alarm settings and exit the menu
{
    printf ("Save Alarm");
...
void Initialize_Setup_Alarm_Time_Screen(void)               // LCD screen
{ 
...
void Update_Setup_Alarm_Time_Screen(void)
{
    ...
    setup_alarm_time[0]  = (setup_alarm_hour/10)  + '0'; // MSB
    setup_alarm_time[1]  = (setup_alarm_hour%10)  + '0';
    setup_alarm_time[3]  = (setup_alarm_minute/10)+ '0'; // MSB
    setup_alarm_time[4]  = (setup_alarm_minute%10)+ '0';
    setup_alarm_time[6]  = (setup_alarm_second/10)+ '0'; // MSB
